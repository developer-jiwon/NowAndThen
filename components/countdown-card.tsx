...
export default function CountdownCard({
  ...
  return (
    ...
    >
      ...
      <div className="absolute -top-3 -right-1 z-20 flex flex-row gap-0.5 bg-white/95 rounded-2xl shadow-lg px-1 py-0.5 border border-gray-100"
      ...
      >
        ...
        >
          {category === "hidden" ? <Eye className="w-2.5 h-2.5 sm:w-3 sm:h-3" /> : (countdown.hidden ? <Eye className="w-2.5 h-2.5 sm:w-3 sm:h-3" /> : <EyeOff className="w-2.5 h-2.5 sm:w-3 sm:h-3" />)}
        </div>
        {onEdit && (
          <div
